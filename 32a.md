...
*Contributed by Jason Yee*.Suppose the following structure represented a node in a binary tree: `struct Node {
    int value;
    Node* left;
    Node* right;
...
We go through each node one by one and record each node's reference (or memory address) in a hash table. If the current node is null, we have reached the end of the list and it must not be cyclic. If current node’s reference is in the h\
ash table, then return true.
